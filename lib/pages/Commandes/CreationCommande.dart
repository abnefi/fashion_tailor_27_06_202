...
class _CreerCommandeState extends State<CreerCommande> {
  ...
  void _ajoutTenueDialog() {
    showDialog(
        context: context,
        builder: (_) {
          return MyDialog(idCommande: widget.idCommande,);
        });
  }
  ...
  void ajoutCommande() {
    ...
    if (form.validate())
    {
      setState(()  {
        ...
        String intitule=monClient.prenomclient+' '+monClient.nomclient;
        if(intitule.length>30)
          {
            intitule=intitule.substring(0,28)+'..';
          }

        ctrCloudCommande.insertDataCommande(Commande(null, widget.idCommande, monClient.idclientFirestore,token,  DateFormat('dd/MM/yyyy').format(currentDate), montantAvancer, DateFormat('dd/MM/yyyy').format(_dateTime1),monClient.avatarclient,intitule, 'Desc comd', 1, 2));
        print('Commande save');
                          ...
                          Container(
                              ...
                              child: Row(
                                children: <Widget>[
                                  IconButton(
                                    icon: Icon(
                                      Icons.calendar_today,color: Colors.lightBlueAccent,
                                    ),
                                    color: Colors.white,
                                    // onPressed: () {
                                    //   _selectDate(context);
                          ...
                          Container(
                              ...
                              child: Row(
                                ...
                                children: <Widget>[
                                  IconButton(
                                    icon: Icon(
                                      Icons.date_range,color: Colors.lightBlueAccent,
                                    ),
                                    color: Colors.white,
                                    onPressed: () {
                                  ...
                                  Text(
                                    _dateTime1 == null ? 'Date livraison' : DateFormat('dd/MM/yyyy').format(_dateTime1) ,
                                    style:
                                    TextStyle(fontSize: 18.0, color: Colors.black54),
                                  )
                                ],
...
class MyDialog extends StatefulWidget {
  ...
  MyDialog({Key key, @required this.idCommande}) : super(key: key);
  @override
